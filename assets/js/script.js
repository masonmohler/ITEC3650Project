...
async function getData() {
  ...
  if (form.valid()) {
    ...
    if (msgObject.status >= 200 && msgObject.status <= 299) {
      ...
      let currentLevel = values[fLen - 1];

      document.getElementById("currentWaterLevel").innerHTML =
      ...
      createChart(values, dates, sitename);
    } else {
...
function createChart(values, dates, sitename) {
